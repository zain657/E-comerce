    ...
    return (
        <div className="flex items-center mt-2.5 mb-5">
            <div className="flex items-center space-x-1 rtl:space-x-reverse">
                {[1, 2, 3, 4, 5].map((rate) => {
                    const starClass = rate <= integerRating
                        ? "w-4 h-4 text-yellow-300"
                        : rate === integerRating + 1 && hasHalfStar
                        ? "fa-solid fa-star-half text-yellow-300"
                        : "w-4 h-4 text-slate-300";
                    return rate === integerRating + 1 && hasHalfStar ? (
                        <i key={rate} className={starClass} aria-hidden="true"></i>
                    ) : (
                        <svg
                            key={rate}
                            className={starClass}
