                                    ...
                                    <div className="grid gap-y-4">
                                        <div>
                                            ...
                                            <div className="relative">
                                                <input type="email" id="email" name="email" value={values.email} onChange={handleChange} onBlur={handleBlur} className="py-3 px-4 block w-full border-2 border-gray-200 rounded-md text-sm focus:border-red-500 focus:ring-red-500 shadow-sm" required aria-describedby="email-error" />
                                                {touched.email && errors.email && <p className="text-red-500">{errors.email}</p>}
                                            </div>
                                            <p className="hidden text-xs text-red-600 mt-2" id="email-error">Please include a valid email address so we can get back to you</p>
                                        </div>
                                        <button disabled={isLoading || (seconds>1 && success)}  type="submit" className="py-3 disabled:bg-gray-400 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-red-500 text-white hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all text-sm dark:focus:ring-offset-gray-800">Send code {isLoading && <i className='fas fa-spin fa-spinner'></i>}</button>
                                        {seconds>0 && success &&<p className='text-gray-500 text-center'>Remaining time: {seconds} seconds</p>}
